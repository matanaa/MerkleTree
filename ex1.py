...
class node:
    def __init__(self,saveData=True):
        self.hash_value = hashlib.sha256(''.encode()).hexdigest()
        self.right = None
        self.left =None
        self.data = None
...
def create_Proof_of_Inclusion(root,id):
    ...
    while father.father !=None :
        hashlist += DELIMITER+Get_Brother_Hash(now)
        now = father
    ...
    return f"{hashlist}{DELIMITER}{root.getHashValue()}"

def verify_Proof_of_Inclusion(node_data,proof):
    proof = proof.split(DELIMITER)
    node_hash = hashlib.sha256(node_data.encode()).hexdigest()
    hash_value = ''
    if proof[0] is not '':
        if proof[0].startswith('0'):
            hash_value = hashlib.sha256(proof[0][1:].encode()+node_hash.encode()).hexdigest()
        else:
            hash_value = hashlib.sha256( node_hash.encode() + proof[0][1:].encode() ).hexdigest()

    for i in range(1,len(proof)-1):
        p = proof[i]
        if p.startswith('0'):
            hash_value = hashlib.sha256(p[1:].encode()+hash_value.encode()).hexdigest()
        else:
            hash_value = hashlib.sha256( hash_value.encode() + p[1:].encode() ).hexdigest()
...
try:
    ...
    while True:
        ...
        if action.strip() == "1":
            ...
            print(f"{root_node.getHashValue()}")

        elif action.strip() == "3":
            print(f"{create_Proof_of_Inclusion(root_node,int(data.strip()))}")

        elif action.strip() == "4":
            print(f"{verify_Proof_of_Inclusion(data.split(DELIMITER, 1)[0],data.split(DELIMITER, 1)[1])}")
            ...
            print(sign_data(data, root_node.getHashValue()).decode())
            ...
            print (verify_data(data.split(DELIMITER)[0], data.split(DELIMITER)[1], data.split(DELIMITER)[2]))
